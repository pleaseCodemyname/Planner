...
app.post("/account/login", async (req, res) => {
  ...
  try {
    ...
    // 여기서 Python 스크립트에 토큰을 전달
    await sendTokenToPythonScript(token);
...
// // Python 스크립트로 토큰 전송하는 함수
// async function sendTokenToPythonScript(token) {
//   try {
//     const pythonScriptURL = 'http://43.201.211.135:8000/receive_token';
//     const response = await axios.post(pythonScriptURL, `token=${token}`, {
//       headers: {
//         'Content-Type': 'application/x-www-form-urlencoded',
//       },
//     });
//     console.log('토큰 전송 결과:', response.data);
//   } catch (error) {
//     console.error('토큰 전송 중 오류:', error);
//   }
// }
